...
public class FileHandling {
    ...
    public FileHandling(SIG.view.InvoiceFrame frame) {
        this.frame = frame;
    }
    ...
    public ArrayList<SIG.model.sigHeader> readFile(){
        String headerPath;
        //items file name
        String itemPath;
        File headerFile ;
        ...
        int result;
        ArrayList<SIG.model.sigHeader> invArray = new ArrayList<>();
        ...
        if(headerLines!=null && itemLines !=null){
            for (String headerLine : headerLines) {
                ...
                SIG.model.sigHeader invoice = new SIG.model.sigHeader(num, customerName, date);
            ...
            for (String itemLine : itemLines) {
                ...
                SIG.model.sigHeader invoice=getInvoiceByNum(num);
                SIG.model.sigItem line = new SIG.model.sigItem(name1,quantity,unitPrice,invoice);
    ...
    public SIG.model.sigHeader getInvoiceByNum(int num){
        for(SIG.model.sigHeader inv : frame.getInvoices()){
            if(inv.getNum()==num){
                return inv;
            }
        }
    ...
    public void saveFile(ArrayList<SIG.model.sigHeader> headers) {
        ...
        */
        for(SIG.model.sigHeader header: headers){
            String headerLines = header.getInvoicesFromTabel();
            invoices=invoices+headerLines;
            invoices=invoices+"\n";

            for(SIG.model.sigItem item : header.getItems()){
            ...
            if(result == JFileChooser.APPROVE_OPTION){
                ...
                if(headerFile.getName().contains(".csv")){//write only in the correct file
                    ...
                    try {
                        ...
                        Logger.getLogger(SIG.model.FileHandling.class.getName()).log(Level.SEVERE, null, ex);
                    } finally {
                        try {
